...
function RepoWatch() {
  ...
  const openPath = (path: string) => { void rpc.call("openPath", { path }).then((result) => { if (!result.ok) setError(result.message); }); };
  const showGitStatus = (path: string) => { setStatusOutput(null); void rpc.call("gitStatus", { path }).then((result) => { if (result.ok) setStatusOutput(result.output || "Working tree clean"); else setError(result.output); }); };
  return <main className="h-full space-y-5 overflow-y-auto p-6">
    ...
    {statusOutput !== null ? <pre role="status" className="overflow-x-auto rounded-md border bg-muted p-3 text-xs">{statusOutput}</pre> : null}
    ...
    <div className="space-y-3">{snapshot?.repos.map((repo) => <section className="rounded-lg border p-4" key={repo.path}><div className="flex items-start justify-between gap-3"><div><h2 className="font-medium">{repo.name}</h2><p className="text-xs text-muted-foreground">{repo.path} · {repo.branch ?? "detached"} · base {repo.defaultBranch}</p></div><span className={repo.isClean ? "text-xs text-muted-foreground" : "text-xs font-medium text-destructive"}>{repo.isClean ? "clean" : `${repo.changedFiles} changed`}</span></div><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-md border px-2 py-1 text-xs" onClick={() => openPath(repo.path)}>Open repo</button><button className="rounded-md border px-2 py-1 text-xs" onClick={() => showGitStatus(repo.path)}>Show git status for {repo.name}</button></div>{repo.error ? <p className="mt-3 text-sm text-destructive">{repo.error}</p> : null}{repo.unmergedBranches.length ? <p className="mt-3 text-sm">Unmerged: {repo.unmergedBranches.join(", ")}</p> : null}<div className="mt-3 space-y-2">{repo.worktrees.map((worktree) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-md bg-muted p-3 text-sm" key={worktree.path}><span className="min-w-0 break-all">{worktree.path} <span className="text-xs text-muted-foreground">{worktree.isMissing ? "missing" : worktree.isMain ? "main worktree" : worktree.needsCleanup ? "merged and clean" : worktree.branch ?? "detached"}</span></span><div className="flex shrink-0 flex-wrap gap-2"><button className="text-xs underline" onClick={() => openPath(worktree.path)}>Open worktree</button><button className="text-xs underline" onClick={() => showGitStatus(worktree.path)}>Status</button>{worktree.needsCleanup ? worktree.isMissing ? <button className="text-xs underline" onClick={() => prune(repo.path)}>Prune</button> : <button className="text-xs underline" onClick={() => remove(repo.path, worktree.path)}>Remove</button> : null}</div></div>)}</div></section>)}</div>
  </main>;
}
